...
export default function Register() {
  ...
  const googleSignup = useGoogleLogin({ onSuccess: handleGoogle });
  ...
  return (
    <AuthLayout title="Create account" subtitle="Start syncing your clipboard across all devices.">
            ...
            <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
              {[1, 2, 3].map(n => (
                <div key={n} style={{
                  ...
                  background: strength >= n ? strengthColor[strength] : 'var(--border)',
                  transition: 'background 0.3s ease'
                }} />
              ))}
              <span style={{ fontSize: 11, color: strengthColor[strength], marginLeft: 4, whiteSpace: 'nowrap' }}>
      ...
      <button className="btn btn-secondary btn-lg" style={{ width: '100%', gap: 10 }} onClick={() => googleSignup()} disabled={loading}>
        <GoogleIcon />
        Sign up with Google
      </button>
